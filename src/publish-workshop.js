...
(async () => {
  ...
  const details = {
    ...
    changeNote: 'v0.9.84-beta — NEW: HOST OVER THE INTERNET, DIRECTLY. Steam routes co-op traffic through its own relay, which throttles bandwidth and pushed ping into the seconds (measured 3000 ms) — that is what produced the holes in the world and the terrible sync many of you reported. The lobby now has Host (Internet - direct): the mod asks your router to open the port by itself (UPnP) and shows your address, MASKED by default with show / hide and a copy button that never reveals it on screen, so it is safe to stream. Your friend pastes that address into Join LAN. Bandwidth was also cut to a quarter of what it was, with the rate now reacting to ping instead of only to backlog. FIXED: the false MOD VERSION MISMATCH warning when connecting from the menu lobby (report: Cr0ss0vr), a false OLD mod warning shown to up-to-date players, and a stale no-data-from-host message that stayed on screen after the data came back. Auto-updates at next launch; if you installed between Aug 18 and Aug 20, run install.bat ONCE first.',
    previewPath: PREVIEW,
    contentPath: CONTENT,
    visibility: vis,
    tags: ['Mods'],
